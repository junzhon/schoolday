...
public class InputDealing {
    ...
    private static List<String> exp2zhong(String expression) throws DealingException{
        ...
        for(String s : expression.split(regex)){
            ...
            if(InputCheck.CheckNum(s) || InputCheck.CheckOperator(s) > 0 || s.equals("(") || s.equals(")")){
                a.add(s);
            }else{
    ...
    public static String dealing(String expression){
        String ss = null;
    ...
    try{
        ...
        if(zhong.size() == 0){
            ...
            if(InputCheck.CheckNum(check) || InputCheck.CheckOperator(check) > 0 ){
	...
	public static List<String> zhongToHou(List<String> zhong) throws DealingException{
        ...
        for (int i = 0; i < zhong.size();) {
            ...
            if (InputCheck.CheckNum(s)) {
                ...
                if (s1.empty() || s1.peek().equals("(") || Priority.priorty(s) >= Priority.priorty(s1.peek())) {
                    s1.push(s);
                    i++;
                } else {
